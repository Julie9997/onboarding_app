...
public class UserEdit extends StandardEditor<User> {
    ...
    public void onGenerateButtonClick(Button.ClickEvent event) {
        User user = getEditedEntity(); //метод базового класса StandardEditor, чтобы получить редактируемого пользователя

        if (user.getJoiningDate() == null) { //Если атрибут joiningDate не установлен, показать сообщение и завершить работу
        ...
        List<Step> steps = dataManager.load(Step.class) // Загрузить список зарегистрированных шагов
            .query("select s from Step s order by s.sortValue asc")
            .list();

        for (Step step : steps) {
            if (stepsDc.getItems().stream().noneMatch(userStep ->
                userStep.getStep().equals(step))) { //Пропустить Step, если он уже находится в контейнере коллекции stepsDc
                UserStep userStep = dataContext.create(UserStep.class); //Создать новый экземпляр UserStep, используя метод DataContext.create()
                userStep.setUser(user);
                userStep.setStep(step);
                userStep.setDueDate(user.getJoiningDate().plusDays(step.getDuration()));
                userStep.setSortValue(step.getSortValue());
                stepsDc.getMutableItems().add(userStep); //Добавить новый экземпляр UserStep в контейнер коллекции stepsDc, чтобы отобразить его в пользовательском интерфейсе
            }
        }
    }
